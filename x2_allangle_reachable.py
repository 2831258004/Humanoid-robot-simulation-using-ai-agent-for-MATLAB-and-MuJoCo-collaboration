...
def main():
    ...
    # 1. 批量加载 MATLAB 数据集
    # ==========================================
    mat_path = 'X2_Arm_collosion_Workspace_Data_angle.mat'
    print(f"正在加载数据集: {mat_path} ...")
    try:
        mat_data = scipy.io.loadmat(mat_path)
        valid_pos = mat_data['valid_pos']       # 形状: (3, N)
        valid_angles = mat_data['valid_angles'] # 形状: (5, N)
    except FileNotFoundError:
        print(f"❌ 找不到文件 {mat_path}，请检查路径！")
        return

    num_points = valid_pos.shape[1]
    print(f"✅ 成功加载，共计 {num_points} 个工作空间数据点。\n")
    ...
    for i in range(num_points):
        # 提取当前点的目标位置和关节角
        target_xyz = valid_pos[:, i]
        target_q = valid_angles[:, i]
    ...
    if max_error_mm < 1.0:
        print("🎉 完美！所有点位的数学空间均与底层物理空间严丝合缝对齐！")
    else:
    ...
    plt.hist(errors_mm, bins=50, density=True, color='#1f77b4', alpha=0.6, edgecolor='black', linewidth=0.5, label='Error Frequency')
    
    # 5.2 计算并绘制核密度估计曲线 (KDE - Probability Density Function)
    # 只有当误差存在分布差异时才绘制 KDE，防止所有误差都是 0.0000 导致报错
    ...
    # 5.3 绘制平均误差垂直参考线
    plt.axvline(x=mean_error_mm, color='red', linestyle='--', linewidth=2, 
                label=f'Mean Error: {mean_error_mm:.4f} mm')

    # 5.4 图表装饰与排版
    plt.title('Error Distribution Histogram (MATLAB vs MuJoCo)', fontsize=20, fontweight='bold', pad=15)
    plt.xlabel('Kinematic Error Distance (mm)', fontsize=20)
    plt.ylabel('Probability Density', fontsize=20)

    # 2. 更改 X 轴和 Y 轴上【刻度数字】的字体大小 (labelsize)
    plt.tick_params(axis='both', which='major', labelsize=25) 
    # 3. 强制设置刻度数字的小数位数
    # 注释掉原来的科学计数法：# plt.ticklabel_format(...)
    # '%.4f' 表示保留 4 位小数，您可以改成 '%.6f' (6位) 或 '%.2f' (2位)
    plt.gca().xaxis.set_major_formatter(FormatStrFormatter('%.4f')) 
    plt.gca().yaxis.set_major_formatter(FormatStrFormatter('%.1f'))
    # 使用科学计数法或者保留足够小数位，防止 X 轴标签重叠
